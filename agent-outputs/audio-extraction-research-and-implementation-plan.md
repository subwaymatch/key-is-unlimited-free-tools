...
Both cores are **above Cloudflare's 25 MiB static-asset limit** (see §4.2).
...
| Speed on codec-bound work | baseline | ~2× faster |
...
- **Stream copy (lossless, fast):** `-i input -vn -acodec copy out.m4a` — demux + remux only, no decode/encode. Bounded by WORKERFS read speed; realistically tens of seconds to a few minutes for multi-GB files, not the ~25× WASM slowdown that applies to encoding.
- **Re-encode (format conversion):** `-i input -vn -c:a libmp3lame -q:a 2 out.mp3` etc. This is where WASM's slowdown is felt — a long video can take minutes. Live progress (§6.5) matters most here.
...
| Path | Fit |
|---|---|
| **`output: 'export'` → static assets** | ✅ App is 100% client-side; every response is a free static asset; simplest, no server runtime, `_headers` applies to all responses |
...
This matters for two reasons: as **insurance** against ffmpeg.wasm's caretaker-mode risk and Safari unknowns, and for the one **output-size edge case** (§3.3) that MEMFS can't handle.
...
|---|---|---|---|
| >2 GB **input** | ✅ (verified 13.2 GB) | ✅ (streaming by design) | ✅ (device I/O) |
| >2 GB **output** | ❌ MEMFS cap | ✅ StreamTarget | ✅ |
| Container coverage | Widest (all of ffmpeg) | MP4/MOV/MKV/WebM/TS/… (no AVI/WMV) | Wide (build-dependent) |
| Works without WebCodecs | ✅ | Copy-only paths | ✅ |
| Download size | ~31 MiB core | ~5–200 kB | 1.5–3 MiB |
| Maintenance | ⚠️ caretaker mode | ✅ very active | ✅ active |
...
```ts
// coreLoader.ts — core is >25 MiB, so it CANNOT be a static asset (§4.2)
...
async function extract(ffmpeg: FFmpeg, file: File, fmt: Format,
  ...
  try {
    const dur = await probeDuration(ffmpeg, `/mnt/${file.name}`); // §6.5
...
| **WAV** | `-c:a pcm_s16le` | encode | See size guard §6.7 |
...
2. On drop, each file becomes a card and the queue starts immediately (requirement: auto-start). The engine loads on first use, with the download progress bar from §6.5.
...
- **Input >2 GB:** handled structurally by WORKERFS (§3.3) — no special casing needed, but Phase 0 verifies it on real 3–8 GB files in Chrome, Firefox, Safari.
- **Output size guard:** before a WAV job, estimate size (`duration × sampleRate × channels × 2 B`); if the estimate approaches ~1.5 GB, warn and suggest FLAC (lossless, ~50–60% smaller) instead. This is the only spot where the MEMFS output cap can realistically bite.
- **Memory hygiene:** unmount + delete outputs after each job; revoke object URLs on card removal; one job at a time.
- **Safari:** dedicated test pass for WORKERFS memory behavior (§3.3); if Safari proves problematic on multi-GB files, gate huge files there behind the Mediabunny fallback engine (§7) or a "use Chrome/Firefox for files over N GB" notice — decision point at Phase 5.
...
| Phase | Scope | Est. |
|---|---|---|
| **0. Feasibility spike** | Throwaway page: load core from CDN, WORKERFS-mount a >4 GB file, stream-copy + MP3-encode, verify encoder list (`-encoders`), measure timings in Chrome/Firefox/Safari. **Go/no-go gate for §3.3's Safari caveat.** | 1–2 d |
...
| **5. Large-file & browser hardening** | 3–8 GB test matrix (Chrome/Firefox/Safari/Edge, one mobile pass), memory profiling, Safari decision (§6.7), copy-mode container mapping edge cases | 2–3 d |
...
|---|---|---|
| ffmpeg.wasm abandonment | Medium | Pinned versions + blob-loaded core keep working indefinitely; `AudioExtractor` interface makes a Mediabunny swap a contained change (§7) |
| Safari memory growth on WORKERFS multi-GB reads | Medium | Phase 0/5 testing; per-browser gate or fallback engine |
| WAV output > MEMFS cap on very long videos | Low | Pre-flight size estimate + FLAC suggestion (§6.7) |
| jsDelivr outage/blocking | Low | R2 mirror behind own domain (§6.8) |
...
| Built-in `progress` event garbage values | High (known) | Computed ratio from probed duration + `time` (§6.5) |
